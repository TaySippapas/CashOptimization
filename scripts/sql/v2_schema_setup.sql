-- Schema for the /api/v2 backend (server/uc_repo_v2.py).
--
-- Derived from the SELECT statements the running code issues, NOT from
-- database_table_blueprint.md — the blueprint documents 11 tables and omits
-- dim_route_parameter, and scripts/sql/ktb_cash_route_setup.sql builds an
-- older 7-table design (dim_route, fact_denomination_gap) that v2 never
-- queries. Every column below is one the backend actually reads.
--
-- Set catalog/schema to match V2_CATALOG / V2_SCHEMA (defaults: mdp_dev_dit / default).
...
-- Undocumented in database_table_blueprint.md but required by the Route Config
-- page. Read AND written: POST /api/v2/route-params updates value.
