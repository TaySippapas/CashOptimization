# Upstream database table blueprint

This preserves the workbook's source mappings and its 11-table model. It is
an upstream reference, not an executable definition of the current app schema.
The [runtime contract](runtime-schema.md) identifies the 12 tables used by V2.

## Differences from the runtime schema

- The workbook's 11 tables overlap with the app's branch, machine, and route
  domains. Runtime setup additionally requires `dim_route_parameter` (six fields:
  `parameter_type`, `parameter`, `description`, `value`, `remark`, `updated_at`).
- Runtime column names/types and required subsets are defined by the
  [baseline DDL](../../scripts/sql/schema/v2_schema_setup.sql) plus
  [cost-of-fund migration](../../scripts/sql/migrations/v2_add_cost_of_fund.sql).
  Workbook field counts below describe the upstream workbook, not those DDL tables.
- `dim_truck.is_available` supports fleet updates. The archived truck master
  using `status` and `base_depot_id` is not a replacement for that runtime table.
- V2 reads `fact_route_stop`; it does not require a `fact_route_stop_denomination`
  table. Older README text incorrectly listed that table.
- The earlier `dim_route` / `fact_denomination_gap` seven-table design is archived.
  Current setup uses `fact_route_summary` and per-domain denomination tables.
