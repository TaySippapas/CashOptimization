> Archived reference for an earlier pipeline. See the [archive index](README.md) and [current runtime contract](../data/runtime-schema.md).
...
> with separate branch/machine tables. See [`database_table_blueprint.md`](../data/database-table-blueprint.md)
...
> working, re-runnable SQL lives at `scripts/legacy/sql/ktb_cash_route_setup.sql`
> (run via `scripts/run_ktb_cash_route_setup.py`); the API is
> `server/uc_repo_v2.py` + `server/api/v2.py`, mounted in `app.py`.
...
|---|---|
| No `app_settings.csv` (region defaults: fleet size, cost/km, CO₂/km, service times, thresholds) | Hardcode the same defaults already used in `scripts/legacy/seed_uc_mock_tables.py` (`available_trucks=3`, `average_speed_kmh=62`, `cost_per_km_thb=65`, `co2_per_km_kg=0.27`, `crew_per_vehicle=2`, `city_service_minutes=25`, `district_service_minutes=35`) into a manual `INSERT` — **tell me if you have a real values source, or edit the numbers below** |
...
-- the other 7 staging tables. Full working version: scripts/legacy/sql/ktb_cash_route_setup.sql
```
...
forecast) — see `scripts/legacy/sql/ktb_cash_route_setup.sql`.
...
vars, doesn't touch `server/settings.py`) + `server/api/v2.py` (Pydantic
...
Same mechanics as [`DEPLOY.md`](../deployment.md):
